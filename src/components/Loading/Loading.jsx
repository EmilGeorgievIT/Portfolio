...
export default class Loading extends Component {
    ...
    deadlineAnimation = () => {
        let designerArmCrop = document.getElementById('designer-arm-grop');

        setTimeout(function(){
            designerArmCrop.style.animationDuration = '1.5s';
        },0);

        setTimeout(function(){
            designerArmCrop.style.animationDuration = '1s';
        }, 4000);

        setTimeout(function(){
            designerArmCrop.style.animationDuration = '0.7s';
        }, 8000);

        setTimeout(function(){
            designerArmCrop.style.animationDuration = '0.3s';
        }, 12000);

        setTimeout(function(){
            designerArmCrop.style.animationDuration = '0.2s';
        }, 15000);
    }

    deadlineText = () => {
        let el = document.querySelector('.deadline-days');
        let html = '<div class="mask-red"><div class="inner">' + el.innerHTML + '</div></div><div class="mask-white"><div class="inner">' + el.innerHTML + '</div></div>';    
        el.innerHTML = html;
    }
    ...
    timer = (totalTime, deadline) => {
        ...
        function countTime() {
          --actualDay;
          document.getElementsByClassName('day')[0].textContent = actualDay;
    
          if (actualDay === 0) {
            clearInterval(timer);
            document.getElementsByClassName('day')[0].textContent = deadline;
          }
        }
    }

    componentDidMount() {
        const { animationTime, days } = this.state;
        
        let progressTimeFill = document.getElementById('progress-time-fill');
        let deathGroup = document.getElementById('death-group');
        progressTimeFill.style.animationDuration = animationTime + 's';
        deathGroup.style.animationDuration = animationTime + 's';

        this.deadlineText(); 
        this.deadlineAnimation();
        this.timer(animationTime, days);
    }
            ...
            <div className="deadline">
                <div className="deadline__content">
                    ...
                    <div className="deadline-days">
                        Deadline <span className="day">7</span> <span className="days">days</span>
                    </div>
                </div>
          </div>    
        )
